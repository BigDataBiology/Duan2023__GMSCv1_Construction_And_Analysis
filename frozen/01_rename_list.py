'''
Concept:
Generate originalname - 100AA rename list.
Peptides are named: >GMSC10.100AA.XXX_XXX_XXX_XXX
...
def sort(infile,outfile,n,prefix):
    ...
    with open(outfile,"wt") as out:
        for i in range (len(seqnumber_list)):
            nf = f'{n:012}'
            out.write(f'{sortseqnumber_list[i][0]}\t{sortseqnumber_list[i][1]}\t{prefix}.{nf[:3]}_{nf[3:6]}_{nf[6:9]}_{nf[9:]}\n')
            n += 1
...
def rename_singleton(infile1,infile2,outfile,n,prefix):
    ...
    for ID,seq in fasta_iter(infile2):
        if ID in name:
            nf = f'{n:012}'
            out.write(f'{ID}\t{prefix}.{nf[:3]}_{nf[3:6]}_{nf[6:9]}_{nf[9:]}\n')
            n += 1
    out.close()          
